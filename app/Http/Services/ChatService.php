...
class ChatService
{
    ...
    /**
     * Get session chat history for LangChain format
     */
    public function getSessionHistoryForLangChain($sessionId)
    {
        try {
            $user = Auth::user();
            ...
            foreach ($history as $record) {
                ...
                if (isset($historyData['messages']) && is_array($historyData['messages'])) {
                    foreach ($historyData['messages'] as $message) {
                        // Convert to LangChain message format
                        if (isset($message['role']) && isset($message['content'])) {
                            $langchainMessages[] = [
                                'type' => $this->mapRoleToLangChainType($message['role']),
                                'content' => $message['content'],
                                'timestamp' => $historyData['timestamp'] ?? $record->created_at->toISOString()
                            ];
                        }
                    }
                }
            ...
            Log::error('Failed to get session history for LangChain', [
                'error' => $e->getMessage(),
                'session_id' => $sessionId,
                'user_id' => Auth::id()
            ]);
    ...
    /**
     * Map OpenAI role to LangChain message type
     */
    private function mapRoleToLangChainType($role)
    {
        switch ($role) {
            case 'user':
                return 'human';
